const image_tags = require('../models/image_tags.model');
const rest = require('../utils/restware');
module.exports = {
    getAll: async function (req, res) {
        try {
            const results = await image_tags.findAll({
                attributes: ['image_id', 'tag_id', 'tagged_at'],
                raw: true,
            });
    ...
    getOne: async function (req, res) {
        ...
        try {
            const attributes =['image_id', 'tag_id', 'tagged_at'];
            const where = { image_id: id };
            const result = await image_tags.findOne({
                where: where,
    ...
    update: async function (req, res) {
        ...
        try {
            const result = await image_tags.update(updatedData, { where: { image_id: id } });
            if (result[0] === 1) {
    ...
    delete: async function (req, res) {
        const id = req.params.id;
        try {
            const result = await image_tags.destroy({
                where: { image_id: id }
            });
            if (result) {
    ...
    create: async function (req, res) {
        const {image_id,tag_id } = req.body;
        if (!image_id || !tag_id) {
        ...
        try {
            ...
            const result = await image_tags.create(news);
